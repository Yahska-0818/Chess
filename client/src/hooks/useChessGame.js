...
export default function useChessGame(options = {}) {
    ...
    async (row, col) => {
      ...
      try {
        if (selectedPiece) {
          ...
          const canMove =
            isLegal &&
            (!roomCode || (playerColor && game.turn === playerColor));
          ...
          if (canMove) {
            ...
            if (piece && (!roomCode || piece.color === playerColor)) {
    ...
    async (moveIndex) => {
      ...
      try {
        if (typeof gameService.getGameSnapshotAtMove === "function") {
          const snapshot = await gameService.getGameSnapshotAtMove(
            game._id,
            moveIndex
          );
          if (snapshot) {
  ...
  const applyExternalState = useCallback((nextState) => {
    ...
    setPromotionData(null);
    setCurrentMoveIndex(
      nextState.moveHistory?.length ? nextState.moveHistory.length - 1 : null
    );
  }, []);
