...
export default function CapturedPieces({ pieces }) {
  ...
  return (
    <div className="flex flex-wrap gap-1 items-center min-h-[32px] bg-emerald-700 p-1 rounded-lg border border-white/5 justify-center">
      {pieces.map((p, i) => {
        const typeKey = typeMap[p.type] || p.type; 
        const colorKey = p.color === 'w' ? 'white' : 'black';
        
        return (
          <div key={i} className="w-6 h-6 opacity-90 transform scale-90 flex items-center justify-center">
