...
const populateInitialBoard = (boardMatrix) => {
  for (let row = 0; row < 8; row++) {
    for (let col = 0; col < 8; col++) {
      if (row == pieces.black.pawn.startingPosition[0]) {
        boardMatrix[row][col] = pieces.black.pawn.icon
      } else if (row == pieces.white.pawn.startingPosition[0]) {
        boardMatrix[row][col] = pieces.white.pawn.icon
      } else if (row == pieces.black.king.startingPosition[0] && col == pieces.black.king.startingPosition[1]) {
        boardMatrix[row][col] = pieces.black.king.icon
      } else if (row == pieces.white.king.startingPosition[0] && col == pieces.white.king.startingPosition[1]) {
        boardMatrix[row][col] = pieces.white.king.icon
      } else if (row == pieces.black.queen.startingPosition[0] && col == pieces.black.queen.startingPosition[1]) {
        boardMatrix[row][col] = pieces.black.queen.icon
      } else if (row == pieces.white.queen.startingPosition[0] && col == pieces.white.queen.startingPosition[1]) {
        boardMatrix[row][col] = pieces.white.queen.icon
      } else if ((row == pieces.black.rook.startingPosition[0][0] && col == pieces.black.rook.startingPosition[0][1]) || (row == pieces.black.rook.startingPosition[1][0] && col == pieces.black.rook.startingPosition[1][1])) {
        boardMatrix[row][col] = pieces.black.rook.icon
      } else if ((row == pieces.white.rook.startingPosition[0][0] && col == pieces.white.rook.startingPosition[0][1]) || (row == pieces.white.rook.startingPosition[1][0] && col == pieces.white.rook.startingPosition[1][1])) {
        boardMatrix[row][col] = pieces.white.rook.icon
      } else if ((row == pieces.black.knight.startingPosition[0][0] && col == pieces.black.knight.startingPosition[0][1]) || (row == pieces.black.knight.startingPosition[1][0] && col == pieces.black.knight.startingPosition[1][1])) {
        boardMatrix[row][col] = pieces.black.knight.icon
      } else if ((row == pieces.white.knight.startingPosition[0][0] && col == pieces.white.knight.startingPosition[0][1]) || (row == pieces.white.knight.startingPosition[1][0] && col == pieces.white.knight.startingPosition[1][1])) {
        boardMatrix[row][col] = pieces.white.knight.icon
      } else if ((row == pieces.black.bishop.startingPosition[0][0] && col == pieces.black.bishop.startingPosition[0][1]) || (row == pieces.black.bishop.startingPosition[1][0] && col == pieces.black.bishop.startingPosition[1][1])) {
        boardMatrix[row][col] = pieces.black.bishop.icon
      }
      else if ((row == pieces.white.bishop.startingPosition[0][0] && col == pieces.white.bishop.startingPosition[0][1]) || (row == pieces.white.bishop.startingPosition[1][0] && col == pieces.white.bishop.startingPosition[1][1])) {
...
function App() {
  ...
  const pieceColor = chessBoard[row][col].props.className.slice(0,5)
  const newRow = pieceColor=="white" ? row - 2 : row + 2
  const newCol = col - 1
